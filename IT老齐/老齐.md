...
影响版本：Apache Log4j 2.x rc1版本
存在递归解析功能，可以构造恶意请求，触发远程代码执行漏洞，无需特殊配置。
Struct2、Solr、Druid、Flink均受影响
解决方案：升级到2.15.0-rc2最新版本

SpringBoot日志默认基于slf4j和logback实现日志管理，使用facade模式统一接口。
![](IT%E8%80%81%E9%BD%90/attachments/2936a6f9e9b8b487ee5bec7e30664b8c_MD5.jpeg)
针对简单的，敏捷开发的程序，使用SpringBoot自带配置logging
![](IT%E8%80%81%E9%BD%90/attachments/e741d8c73d0f5c57c9fa8ece4f363236_MD5.jpeg)
针对复杂的，创建logback-spring.xml

## 85 海量文件存储，GIS文件检索系统的架构考量
需求：
1.公司希望开发一套文件检索系统，承载海量地理信息文件，提供多维度的数据检索，要求这个系统能提供高可用与高并发性能
2.每一组GS地图瓦片（分片）都是JSON文件（元数据）+若干图片+二进制地理数据文件，尺寸均小于100K
3.这些地理数据比较稳定，不定期会产生小规模更新
4.目前积累的数据总量约为1T,目测文件数量最少上干万（保守）
5.应用UV很低，日均100~200用户访问，但QPS并发量很高，因为一张地图要切分几十（百）个瓦片，在访问时并发获取。
...
初步：
1.MySQL中不中？存储解析JSO在MySQL8中虽然支持，但总感觉不是它的专业呀
2.MongoDB可以自动解析JSON,如果基于MongoDB集群保障数据可靠性应该中~
3.存储文件的硬盘绝对不能用机械盘，上干万文件的这种随机读光寻址要死人的
4.SSD是目前是随机读好选择，就是存储成本有点高
5.存储高可用怎么办？我觉得Raid10(5)不错，1T存储空间不大，当下UV极小，这点QPS在IO上不叫事，而且属于服务器本机部署，不用考虑带宽问题，但所有磁盘都挂载在一台服务器上在物理上会形成单点
![](IT%E8%80%81%E9%BD%90/attachments/c1fb1225de198dc63625ded94b78faa0_MD5.jpeg)
左侧Raid直接与应用服务器物理连接，不需要考虑网络带宽问题，但是应用服务器会有单点问题，并且存储难以横向拓展。
可以创建文件服务器；
![](IT%E8%80%81%E9%BD%90/attachments/b0354f41ef390a516f0122a027a05ac1_MD5.jpeg)
但是也会存在Linux文件服务器的单点问题。
考虑：
6.SAN(存储区域网络)这是好东西，相当于内网的百度网盘，高可靠高性能，信息共享方便，就
是比Raid贵N倍，公司肯定不会在这方面有足够预算
7.开源的FastDFS是目前可行的低成本方案（相对），可以考虑采购N\*2+3台普通性能的SSD硬盘服务器，这种方案水平扩容相对轻松，数据共享也比较方便，就是需要额外考虑带宽问题。
8.为什么不考虑Hadoop HDFS?那玩意存大文件还行，存小文件效率比FastDFS差远了
![](IT%E8%80%81%E9%BD%90/attachments/8fd9f0e8769be6c745c9a19ea8f26a70_MD5.jpeg)
==大文件适合HDFS、小文件适合FastDFS==
![](IT%E8%80%81%E9%BD%90/attachments/9717c9800d50a3addb3778a952ba1938_MD5.jpeg)
10.带宽怎么算？假设1块地图平均需要25个瓦片数据，每个瓦片100KB,如果处理时间相同的而情况下，数据同时返回，25\*100KB\*8=20,000kbps,每个用户需要20m的带宽，这在局域网兴许可以，但在广域网传输显然带宽占用太吓人，还需要额外考虑资源压缩传输。

## 86 100w用户、800w流量在线贺卡应用架构如何优化
