...
SPI 即 Service Provider Interface ，字面意思就是：“服务提供者的接口”，我的理解是：专门提供给服务提供者或者扩展框架功能的开发者去使用的一个接口。
- 当接口存在于调用方这边时，这就是 **SPI** 。由接口调用方确定接口规则，然后由不同的厂商根据这个规则对这个接口进行实现，从而提供服务。
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/ab9215d6a8d42cdf135b977e88fbf56b_MD5.jpeg)
...
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/f95575fb11f4536eaa2da776ef968a94_MD5.jpeg)
这就是依赖 SPI 机制实现的。
通过使用 SPI 机制，可以看出服务（`LoggerService`）和 服务提供者两者之间的耦合度非常低，如果说我们想要换一种实现，那么其实只需要修改 `service-provider` 项目中针对 `Logger` 接口的具体实现就可以了，只需要换一个 jar 包即可，也可以有在一个项目里面有多个实现。
**ServiceLoader**
想要使用 Java 的 SPI 机制是需要依赖 `ServiceLoader` 来实现的，ServiceLoader是**一种加载服务实现的工具**。
...
```
解决第三方类加载的机制是通过**线程上下文类加载器**，允许应用程序或容器（如 Web 应用服务器）设置这个类加载器，以便核心类库能够通过它来加载应用程序类。
...
```
**字符串的 switch 是通过`equals()`和`hashCode()`方法来实现的**。先通过hash值判断，然后用equals方法进行安全检查，防止hash碰撞现象的产生。

**泛型**
通常情况下，一个编译器处理泛型有两种方式：`Code specialization`和`Code sharing`。C++和 C#是使用`Code specialization`的处理机制，而 Java 使用的是`Code sharing`的机制。
> **Code specialization** 为**每种泛型实参都生成一份独立的机器码**；**Code sharing** 只生成**一份通用机器码**，通过**类型擦除**让所有实参共享
...
> 这里的 **“transient” 修饰方法** 并不是 Java 语言真正的语义，而是 **反编译工具（如 javassist）在字节码层面复用了同一个标志位数值** 来表示 **「可变参数(varargs)」**，导致反编译结果里错误地显示了 `transient`；**transient 只对成员变量有意义，对方法无效**。
...
内部类之所以也是语法糖，是因为它仅仅是一个编译时的概念，`outer.java`里面定义了一个内部类`inner`，一旦编译成功，就会生成两个完全不同的`.class`文件了，分别是`outer.class`和`outer$inner.class`。所以内部类的名字完全可以和它的外部类名字相同。
 尝试对OutterCLass.class文件进行反编译的时候，会将OutterClass.class与OutterClass$InnerClass.class两个文件全部进行反编译，一起生成一个OutterClass.jad文件，内容如下：
 ```java
...
```
编译完成后，inner实例内部会有指向outer实例的引用this$0，并且outer中会有一个桥方法static String access$000(OutterClass)，返回String，即userName属性。所以反编译后的printOut()方法如下：
```java
public void printOut() {
...
void outerMethod() {
    final int factor = 2;
    class Local {
        int calc(int x) { return x * factor; }
    }
    Local l = new Local();
    System.out.println(l.calc(5)); // 10
}
```
静态内部类：带 `static` 的独立类，可当工具类使用，无外部对象依赖。
```java
class Outer {
    static class Helper {
        static void help() { System.out.println("helper"); }
    }
}
Outer.Helper.help(); // 无需外部对象
```
---
...
**断言**
`assert`关键字是从 JAVA SE 1.4 引入的，为了避免和老版本的 Java 代码中使用了`assert`关键字导致错误，Java 在执行的时候默认是不启动断言检查的（这个时候，所有的断言语句都将忽略！），如果要开启断言检查，则需要用开关`-enableassertions`或`-ea`来开启。
**其实断言的底层实现就是 if 语言，如果断言结果为 true，则什么都不做，程序继续执行，如果断言结果为 false，则程序抛出 AssertError 来打断程序的执行。**`-enableassertions`会设置$assertionsDisabled 字段的值。
...
**try -with-resource**
从 Java 7 开始，jdk 提供了一种更好的方式关闭资源，使用`try-with-resources`语句。
注意其中addSuppressed，可以显示更多的错误信息，至于为什么与字节码有关了。
...
```
有两个重载的函数，因为他们的参数类型不同，一个是`List<String>`另一个是`List<Integer>` ，但是，这段代码是编译通不过的。因为我们前面讲过，参数`List<Integer>`和`List<String>`编译之后都被擦除了，变成了一样的原生类型 List，擦除动作导致这两个方法的特征签名变得一模一样。
- 泛型遇到catch
...
```
输出结果为2！
所有的泛型类实例都关联到同一份字节码上，泛型类的静态变量是共享的。上面例子里的`GT<Integer>.var`和`GT<String>.var`其实是一个变量。
- 自动装箱与拆箱
```java
public static void main(String[] args) {
    Integer a = 1000;
...
Iterator工作在一个独立的线程，又有一个mutex锁。Iterator被创建后会建立一个指向原来对象的索引表，当原来的对象数量发生变化时，索引表的内容不会同步改变，所以被修改后会按照fail-fast原则抛出异常。
所以 `Iterator` 在工作的时候是不允许被迭代的对象被改变的。但你可以使用 `Iterator` 本身的方法`remove()`来删除对象，`Iterator.remove()` 方法会在删除当前迭代对象的同时维护索引的一致性。
> **fail-fast（快速失败）是 Java 集合框架的一种错误检测策略：当迭代器在遍历过程中检测到集合结构被“意外”修改时，立即抛出 `ConcurrentModificationException`，以避免数据不一致或未知行为。**
