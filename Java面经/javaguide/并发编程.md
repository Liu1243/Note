...
如何创建线程池？
- 通过ThreadPoolExecutor构造函数直接创建（推荐）
明确指定线程池的核心参数，对线程池运行行为有更精细的控制，避免资源耗尽的风险。
- 通过Executors工具类创建（不推荐用于生产环境）
...
| -------- | --------------------------- | ----------------------------- |
| **内部结构** | 链表                          | TransferStack / TransferQueue |
| **容量**   | 有限（可指定，默认很大）                | 0（不能存储）                       |
| **数据存储** | 可以缓冲多个元素                    | 直接交接                          |
| **锁机制**  | putLock / takeLock 分离，支持高并发 | 直接交换，模式取决于公平性                 |
| **适用场景** | 生产者-消费者速度不一致，需要缓冲           | 即时移交任务，线程之间点对点                |
| **性能**   | 有额外节点存储和锁开销                 | 轻量级，直接唤醒等待线程                  |
...
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/260ac4109b23f51c1aa2aa67ff98a9d3_MD5.jpeg)

线程池的核心线程会被回收吗？
ThreadPoolExecutor默认不会回收核心线程。如果线程池是被用于周期性使用的场景，且频率不高（周期之间有明显的空闲时间），可以考虑将 `allowCoreThreadTimeOut(boolean value)` 方法的参数设置为 `true`，这样就会回收空闲（时间间隔由 `keepAliveTime` 指定）的核心线程了。
...
- DiscardPloicy：不处理新任务，直接丢弃掉。
- DiscardOldestPolicy：此策略将丢弃最早的未处理的任务请求。

如果不允许丢弃任务，应该选择哪个拒绝策类？
CallerRunsPolicy

CallerRunsPolicy拒绝策略有什么风险？如何解决？
如果走到`CallerRunsPolicy`的任务是个非常耗时的任务，且处理提交任务的线程是主线程，可能会导致主线程阻塞，影响程序的正常运行。
使用CallerRunsPolicy策略是希望所有的任务都能被执行，暂时无法执行的任务存入阻塞队列BlockingQueue中，在内存允许的情况下，我们可以增加阻塞队列`BlockingQueue`的大小并调整堆内存以容纳更多的任务，确保任务能够被准确执行。
为了充分利用 CPU，我们还可以调整线程池的`maximumPoolSize` （最大线程数）参数，这样可以提高任务处理速度，避免累计在 `BlockingQueue`的任务过多导致内存用完。
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/af35976677d4c355c31386e848c3c422_MD5.jpeg)
如果服务器资源达到极限，那么可以进行==任务持久化==：
1. 设计一张任务表将任务存储到 MySQL 数据库中。
2. Redis 缓存任务。
3. 将任务提交到消息队列中。

方案1的图示，需要自定义拒绝策略和阻塞队列：
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/2ce7c51d8ac55d366698781f5941cd04_MD5.jpeg)
...
### 线程池处理任务的流程？
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/1fc08506f8322af32d713a7e6304c35a_MD5.jpeg)
**线程池在提交任务前，可以提前创建线程吗？**
...
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/18ca74cb588d713774a20b6e97123e89_MD5.jpeg)
上面并没有动态指定队列长度的方法，美团的方式是自定义了一个叫做 `ResizableCapacityLinkedBlockIngQueue` 的队列（主要就是把`LinkedBlockingQueue`的 capacity 字段的 final 关键字修饰给去掉了，让它变为可变的）。
可以借助现成的开源项目：
- **[Hippo4j](https://github.com/opengoofy/hippo4j)**：异步线程池框架，支持线程池动态变更&监控&报警，无需修改代码轻松引入。支持多种使用模式，轻松引入，致力于提高系统运行保障能力。
- **[Dynamic TP](https://github.com/dromara/dynamic-tp)**：轻量级动态线程池，内置监控告警功能，集成三方中间件线程池管理，基于主流配置中心（已支持 Nacos、Apollo，Zookeeper、Consul、Etcd，可通过 SPI 自定义实现）。

### 如何设计一个能够根据任务的优先级来执行的线程池？
可以考虑`PriorityBlockingQueue`优先级阻塞队列。
需要让PriorityBlockingQueue实现对任务的排序，任务必须是具备排序能力的：
- 提交到线程池的任务实现Comparable接口，重写compareTo方法执行任务之间的优先级比较规则
- 创建PriorityBlockingQueue时传入一个Comparator对象执行任务之间的排序规则（==推荐==）
这存在一些风险和问题，比如：
- `PriorityBlockingQueue` 是无界的，可能堆积大量的请求，从而导致 OOM。
- 可能会导致饥饿问题，即低优先级的任务长时间得不到执行。
- 由于需要对队列中的元素进行排序操作以及保证线程安全（并发控制采用的是可重入锁 `ReentrantLock`），因此会降低性能。
对于OOM问题，继承PriorityBlockingQueue重写offer方法，当插入元素数量超过指定值就返回 false 。
饥饿问题这个可以通过优化设计来解决（比较麻烦），比如等待时间过长的任务会被移除并重新添加到队列中，但是优先级会被提升。
对于性能方面的影响，是没办法避免的，毕竟需要对任务进行排序操作。并且，对于大部分业务场景来说，这点性能影响是可以接受的。

## Future
> 重点掌握CompletableFuture的使用以及面试题。

`Future` 类是异步思想的典型运用，主要用在一些需要执行耗时任务的场景，避免程序一直原地等待耗时任务执行完成，执行效率太低。
多线程总经典的==Future模式==，核心思想时异步调用。
Future时一个泛型接口，位于JUC包下，定义个5个方法，包括4个功能：
- 取消任务；
- 判断任务是否被取消;
- 判断任务是否已经执行完成;
- 获取任务执行结果。
...
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/b25d44f25a99e7cb1220be3c37614fb4_MD5.jpeg)
...
Java8引入CompletableFuture类解决Future这些缺陷，提供了函数式编程、异步任务编排组合（可以将多个异步任务串联起来，组成一个完整的链式调用）等能力。
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/b25d44f25a99e7cb1220be3c37614fb4_MD5.jpeg)
...
### 使用CompletableFuture，有一个任务失败，如何处理异常？
...
CompletableFuture默认使用全局的ForkJoinPool.commonPllo()作为执行器，所有未指定执行器的异步任务都会使用该线程池。这意味着应用程序、多个库或框架（如 Spring、第三方库）若都依赖 `CompletableFuture`，默认情况下它们都会共享同一个线程池。
为避免这些问题，为CompletableFuture提供自定义的线程池，优势是：
...
AQS是一个抽象类，为同步器提供了通用的执行框架。它定义了 **资源获取和释放的通用流程**，而具体的资源获取逻辑则由具体同步器通过重写模板方法来实现。 因此，可以将 AQS 看作是同步器的 **基础“底座”**，而同步器则是基于 AQS 实现的 **具体“应用”**。
...
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/2c3f1c53b11efe3416252a3d9ccdc912_MD5.jpeg)
...
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/efd2e6c6c072c9d5b390733223822ac4_MD5.jpeg)
AQS的核心原理图：
![](Java%E9%9D%A2%E7%BB%8F/javaguide/attachments/0bf2088d5b8be2cc694b3ec3f548c99c_MD5.jpeg)
...
读取6个文件，没有执行顺序依赖，但是需要返回给用户6个文件处理结果的统计整理。
```java
